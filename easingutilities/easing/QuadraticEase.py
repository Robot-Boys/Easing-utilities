...
class QuadraticEase(AbstractEase):
    ...
    def calculate_next_step(self, current_step, start_value, change_in_value, number_of_steps):
        ...
        if current_step < 1:
            return change_in_value / 2 * current_step * current_step + start_value
        current_step -= 1
        return -change_in_value / 2 * (current_step * (current_step - 2) - 1) + start_value
